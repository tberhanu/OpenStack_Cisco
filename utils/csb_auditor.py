...
def clone_git_repo():
    ...
    try:
        ...
        for filename in os.listdir(os.path.join(root, 'repo')):
            shutil.move(os.path.join(root, 'repo', filename), os.path.join(root, filename))
        return True
        ...
        if os.path.isfile(audit_tc_script + ".py"):
            ...
            try:
                ...
                if results[audit_tc_script] is None:
                    print("LOG: Result for %s was received as None; retrying...")
                    results[audit_tc_script] = tc_script.main(url, team_name, scan_id, team_id)
